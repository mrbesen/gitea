...
func TelegramHooksNewPost(ctx *context.Context, form auth.NewTelegramHookForm) {
	...
	w := &models.Webhook{
		RepoID:          orCtx.RepoID,
		URL:             fmt.Sprintf("%s%s/sendMessage?chat_id=%s", form.ApiUrl, form.BotToken, form.ChatID),
		ContentType:     models.ContentTypeJSON,
		HookEvent:       ParseHookEvent(form.WebhookForm),
		IsActive:        form.Active,
...
func TelegramHooksEditPost(ctx *context.Context, form auth.NewTelegramHookForm) {
	...
	w.Meta = string(meta)
	w.URL = fmt.Sprintf("%s%s/sendMessage?chat_id=%s", form.ApiUrl, form.BotToken, form.ChatID)
	w.HookEvent = ParseHookEvent(form.WebhookForm)
	w.IsActive = form.Active
	if err := w.UpdateEvent(); err != nil {
		ctx.ServerError("UpdateEvent", err)
